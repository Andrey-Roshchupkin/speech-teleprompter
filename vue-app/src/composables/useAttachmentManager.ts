...
export const useAttachmentManager = () => {
  ...
  const parseAttachments = (scriptText: string): void => {
    ...
    while ((match = attachmentRegex.exec(scriptText)) !== null) {
      ...
      const beforeWords = beforeAttachment.split(/\s+/).filter((word) => word.length > 0)
      const attachmentWords = match[0].split(/\s+/).filter((word) => word.length > 0)
   ...
   */
  const convertOriginalToDisplayPosition = (originalPosition: number): number => {
    let displayPosition = 0

    // Check if originalScriptWords is available
    if (!originalScriptWords.value || !Array.isArray(originalScriptWords.value)) {
      return originalPosition
    }

    for (let i = 0; i < originalScriptWords.value.length && i <= originalPosition; i++) {
   ...
   */
  const findAttachmentDisplayPosition = (attachment: Attachment, scriptWords: string[]): number => {
   ...
   */
  const updateAttachmentDisplay = (currentPosition: number, scriptWords: string[]): void => {
    ...
    if (currentPosition < scriptWords.length) {
      ...
      if (currentWord && currentWord.startsWith('[') && currentWord.endsWith(']')) {
    ...
    if (!newCurrentAttachment && attachments.value.length > 0) {
      ...
      for (const attachment of attachments.value) {
        ...
        const displayPosition = findAttachmentDisplayPosition(attachment, scriptWords)
  ...
  const findNextValidPosition = (position: number): number => {
    ...
    for (const attachment of attachments.value) {
      if (position >= attachment.startWordIndex && position < attachment.endWordIndex) {
   ...
   */
  const showFirstAttachment = (scriptWords: string[]): void => {
