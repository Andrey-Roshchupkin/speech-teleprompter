...
export const useSpeechRecognition = (options: UseSpeechRecognitionOptions = {}) => {
  ...
  const resultQueue: SpeechRecognitionResult[] = []
   ...
   */
  const debouncedProcessResult = (newFinalWords: string[], speechOutputHTML: string): void => {
    ...
    resultQueue.push({ newFinalWords, speechOutputHTML })
...
The teleprompter will work in manual mode without speech recognition.
  ...
  const onRecognitionResult = (event: any): void => {
    ...
    const speechOutputHTML =
      finalTranscript.value + '<span class="teleprompter-interim">' + interimTranscript + '</span>'
