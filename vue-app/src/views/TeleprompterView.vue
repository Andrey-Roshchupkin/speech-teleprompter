  ...
  <div class="teleprompter-main-container">
    ...
    <div v-if="!isSpeechSupported" class="browser-warning">
      <div class="warning-content">
        ...
        <p>
          Your browser doesn't support Web Speech API. Speech recognition features are disabled.
        </p>
        ...
        <p>
          <strong>Note:</strong> Web Speech API is not a Baseline feature and has limited browser
          availability.
        </p>
        <p>
          The teleprompter will work in manual mode - you can still edit scripts and use all other
          features.
        </p>
      </div>
    </div>
    ...
    <div class="teleprompter-right-panel">
      ...
      <div class="teleprompter-section">
        ...
        <div class="teleprompter-display" :style="{ height: teleprompterDisplayHeight }">
          <div
        ...
        <!-- Attachment Display -->
        <AttachmentDisplay
          :current-attachment="teleprompterDisplay.attachmentManager.currentAttachment.value"
      ...
      <div class="teleprompter-section">
        ...
        <div class="teleprompter-settings">
          <div class="setting-group">
            ...
            <select id="primaryLanguage" class="language-select" v-model="selectedLanguage">
        ...
        <div class="language-info">
          <p>
            <strong>Language Selection:</strong> Choose your primary language for speech
            recognition.
          </p>
        </div>
      </div>
      ...
      <div class="teleprompter-section">
        ...
        <div class="speech-output">
          <div v-if="!isListening && !hasSpeechOutput" class="speech-placeholder">
          ...
          <div class="setting-column">
            ...
            <div class="radio-group">
              <label class="radio-option">
                <input type="radio" name="logLevel" value="off" v-model="logLevel" />
                <span>Off</span>
              </label>
              <label class="radio-option">
                <input type="radio" name="logLevel" value="error" v-model="logLevel" />
                <span>Error</span>
              </label>
              <label class="radio-option">
                <input type="radio" name="logLevel" value="info" v-model="logLevel" />
                <span>Info</span>
              </label>
              <label class="radio-option">
                <input type="radio" name="logLevel" value="debug" v-model="logLevel" />
                <span>Debug</span>
              </label>
            </div>
          </div>
...
const fuzzyMatcher = useFuzzyMatcher({
  onMatchFound: (result) => {
    ...
    teleprompterDisplay.updatePosition(result.newPosition, result.matchedIndices)
  },
})
...
const updateFirstLineCoordinates = (): void => {
  ...
  firstLineMiddleY.value = firstWordRect.top - containerRect.top + firstWordRect.height / 2
...
const autoScroll = (): void => {
  ...
  if (firstLineMiddleY.value === null || lineHeight.value === null) {
    console.log('🔄 Auto-scroll: First line coordinates not available, updating...')
  ...
  const currentWordElement = teleprompterTextRef.value.querySelector('.teleprompter-highlight')
  if (!currentWordElement) {
    console.log('🔄 Auto-scroll: No highlighted word element found')
    return
  }

  console.log(`🔄 Auto-scroll: Found highlighted element: "${currentWordElement.textContent}"`)
  ...
  const currentWordMiddleY = wordRect.top - containerRect.top + wordRect.height / 2
  ...
  const correction = 1
  const currentLine = Math.floor(lineDifference / (lineHeight.value - correction)) + 1
...
const currentPosition = computed(() => teleprompterDisplay.currentPosition.value)
...
const handleSpeechResult = (result: any): void => {
  // Update speech output display
  finalTranscript.value = speechRecognition.finalTranscript.value
  interimTranscript.value = result.speechOutputHTML.includes('<span class="teleprompter-interim">')
    ? result.speechOutputHTML.split('<span class="teleprompter-interim">')[1].split('</span>')[0]
    : ''
  ...
  if (result.newFinalWords.length > 0) {
    ...
    fuzzyMatcher.processSpokenWords(result.newFinalWords, scriptWords, currentPosition)
  }
}
...
.teleprompter-reset-button {
  background: linear-gradient(45deg, var(--neutral-color), var(--neutral-light));
}
