...
export const useTeleprompterDisplay = (
  options: UseTeleprompterDisplayOptions = {}
) => {
  ...
  const wordStates = computed(() => {
    ...
    const states: Array<'current' | 'matched' | 'passed' | 'normal'> =
      new Array(words.length).fill('normal');
  ...
  const updateScript = (newScriptText: string): void => {
    ...
    const originalWords = newScriptText
      .split(/\s+/)
  ...
  const updateSettings = (
    newSettings: Partial<TeleprompterDisplaySettings>
  ): void => {
