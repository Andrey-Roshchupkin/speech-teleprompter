...
export const useTeleprompterStore = defineStore('teleprompter', () => {
  ...
  const updatePosition = (position: number) => {
    ...
    currentPosition.value = Math.max(0, Math.min(position, scriptWords.value.length - 1))
  ...
  const updateSpeechState = (isListeningState: boolean, status: RecognitionStatus) => {
  ...
  const updateSettings = (settings: {
    ...
    if (settings.linesToShow !== undefined) linesToShow.value = settings.linesToShow
    if (settings.scrollTrigger !== undefined) scrollTrigger.value = settings.scrollTrigger
    if (settings.textSize !== undefined) textSize.value = settings.textSize
    if (settings.fuzzyPrecision !== undefined) fuzzyPrecision.value = settings.fuzzyPrecision
    if (settings.primaryLanguage !== undefined) primaryLanguage.value = settings.primaryLanguage
  }
  ...
  const updateScrollState = (topLine: number, lastScroll: number, count: number) => {
